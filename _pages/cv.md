---
layout: archive
title: "Curriculum Vitæ"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

<div class="cv-page">
  <header class="cv-intro">
    <p class="cv-kicker">Academic profile</p>
    <p class="cv-lead">An overview of my academic background, qualifications and professional profile. Detailed research, teaching, outreach, projects and scientific software are presented on their dedicated pages.</p>
    <div class="cv-actions" aria-label="Download the full curriculum vitae">
      <a class="cv-action cv-action--primary" href="{{ site.url }}/files/cv/CV_ENG.pdf">Full CV · English</a>
      <a class="cv-action" href="{{ site.url }}/files/cv/CV_FR.pdf">CV · Français</a>
    </div>
  </header>

  <div class="cv-highlights" aria-label="Academic highlights">
    <div class="cv-highlight">
      <strong>2023–present</strong>
      <span>Ph.D. candidate at IMAG</span>
    </div>
    <div class="cv-highlight">
      <strong>Ranked 1st</strong>
      <span>I2S doctoral scholarship competition</span>
    </div>
    <div class="cv-highlight">
      <strong>Dual M.Sc.</strong>
      <span>Applied and fundamental mathematics</span>
    </div>
    <div class="cv-highlight">
      <strong>4 languages</strong>
      <span>French, English, Spanish and Japanese</span>
    </div>
  </div>

  <section class="cv-section">
    <div class="cv-section__heading">
      <span>01</span>
      <h2>Current appointment</h2>
    </div>
    <article class="cv-current">
      <div>
        <p class="cv-date">October 2023 — present</p>
        <h3>Ph.D. candidate in Mathematics</h3>
        <p><a href="https://imag.umontpellier.fr">Institut Montpelliérain Alexander Grothendieck</a>, Montpellier, France</p>
      </div>
      <div class="cv-current__meta">
        <p><b>Advisors</b><br><a href="https://vilar.perso.math.cnrs.fr">François Vilar</a> and <a href="https://imag.umontpellier.fr/~marche/">Fabien Marche</a></p>
        <p><b>Funding</b><br>French Ministry of Research fellowship</p>
      </div>
    </article>
  </section>

  <section class="cv-section">
    <div class="cv-section__heading">
      <span>02</span>
      <h2>Education</h2>
    </div>
    <div class="cv-timeline">
      <article>
        <p class="cv-date">2021 — 2023</p>
        <h3>M.Sc. in Theoretical and Numerical Analysis of PDEs</h3>
        <p>University of Montpellier · Highest honours · Valedictorian</p>
      </article>
      <article>
        <p class="cv-date">2020 — 2021</p>
        <h3>M.Sc. in Fundamental Mathematics</h3>
        <p>University of Montpellier</p>
      </article>
      <article>
        <p class="cv-date">2019 — 2020</p>
        <h3>B.Sc. in Pure and Applied Mathematics</h3>
        <p>University of Montpellier · First-class honours</p>
      </article>
      <article>
        <p class="cv-date">2017 — 2019</p>
        <h3>Associate degree in Mathematics &amp; Physics</h3>
        <p>University of Montpellier · First-class honours · Consistently ranked in the top 10%</p>
      </article>
    </div>
  </section>

  <section class="cv-section">
    <div class="cv-section__heading">
      <span>03</span>
      <h2>Recognition</h2>
    </div>
    <div class="cv-grid cv-grid--two">
      <article class="cv-card">
        <p class="cv-card__label">Doctoral fellowship · 2023</p>
        <h3>I2S Doctoral School scholarship</h3>
        <p>Ranked first among all Mathematics candidates in the competitive admission examination for a French Ministry of Research fellowship.</p>
      </article>
      <article class="cv-card">
        <p class="cv-card__label">Competitive support · 2023–2026</p>
        <h3>Doctoral School grants</h3>
        <p>Selected through school-wide calls for computer equipment, international conference travel and support for a Ph.D. defence committee member.</p>
      </article>
    </div>
  </section>

  <section class="cv-section">
    <div class="cv-section__heading">
      <span>04</span>
      <h2>Academic service</h2>
    </div>
    <div class="cv-grid cv-grid--three">
      <article class="cv-card">
        <p class="cv-card__label">Peer review</p>
        <h3>Journal of Computational Physics</h3>
        <p>Ad hoc reviewer.</p>
      </article>
      <article class="cv-card">
        <p class="cv-card__label">2024 — present</p>
        <h3>IMAG Sustainable Development Committee</h3>
        <p>Ph.D. student representative, contributing to institutional discussions and the laboratory’s greenhouse-gas assessment.</p>
      </article>
      <article class="cv-card">
        <p class="cv-card__label">2023 — 2024</p>
        <h3>Graduate student representative</h3>
        <p>Represented students of the Applied Mathematics Master’s programme on its development council.</p>
      </article>
    </div>
  </section>

  <section class="cv-section">
    <div class="cv-section__heading">
      <span>05</span>
      <h2>Professional development</h2>
    </div>
    <div class="cv-training">
      <p><strong>Advanced Programming for Scientific Computing</strong><span>Advanced C++, memory management, scientific data processing and collaborative development.</span></p>
      <p><strong>Creative Pedagogy · Levels 1 &amp; 2</strong><span>Collaborative learning, course design and adaptation to diverse learning contexts.</span></p>
      <p><strong>Management &amp; Leadership</strong><span>Team management, communication, decision-making and leadership posture.</span></p>
      <p><strong>Scientific Writing &amp; Publishing</strong><span>Publication standards, peer review, research integrity and scientific communication.</span></p>
      <p><strong>Research, AI &amp; Ethics</strong><span>Scientific integrity, privacy, bias and responsible decision-making.</span></p>
    </div>
  </section>

  <section class="cv-section">
    <div class="cv-section__heading">
      <span>06</span>
      <h2>Technical profile</h2>
    </div>
    <div class="cv-toolkit">
      <div><h3>Programming</h3><p>C/C++, Python, Fortran</p></div>
      <div><h3>Scientific tools</h3><p>FreeFEM++, Matlab, Scilab, gnuplot, Maple, Mathematica</p></div>
      <div><h3>Scientific writing</h3><p>LaTeX, Beamer, Microsoft Office</p></div>
      <div><h3>Web &amp; systems</h3><p>HTML/CSS, Linux, macOS, Windows</p></div>
    </div>
  </section>

  <section class="cv-section cv-section--last">
    <div class="cv-section__heading">
      <span>07</span>
      <h2>Languages</h2>
    </div>
    <div class="cv-languages">
      <p><strong>French</strong><span>Native · TEF C2</span></p>
      <p><strong>English</strong><span>Fluent · IELTS 7.5</span></p>
      <p><strong>Spanish</strong><span>Intermediate · ≈ B2</span></p>
      <p><strong>Japanese</strong><span>Beginner · ≈ A1</span></p>
    </div>
  </section>
</div>

<style>
.cv-page { --cv-line: rgba(127,127,127,.23); --cv-soft: rgba(127,127,127,.055); }
.cv-intro { max-width: 760px; margin: .2rem 0 1.8rem; }
.cv-kicker, .cv-date, .cv-card__label { margin: 0; color: var(--global-text-color-light); font-size: .66rem; font-weight: 700; letter-spacing: .09em; text-transform: uppercase; }
.cv-lead { margin: .35rem 0 1.1rem; font-size: .95rem; line-height: 1.65; }
.cv-actions { display: flex; flex-wrap: wrap; gap: .55rem; }
.cv-action { display: inline-flex; align-items: center; min-height: 38px; padding: .42rem .82rem; border: 1px solid var(--cv-line); border-radius: 999px; color: var(--global-text-color) !important; font-size: .72rem; font-weight: 600; text-decoration: none !important; transition: transform .18s ease, background .18s ease; }
.cv-action:hover { transform: translateY(-1px); background: var(--cv-soft); }
.cv-action--primary { border-color: var(--global-link-color); color: var(--global-link-color) !important; }
.cv-highlights { display: grid; grid-template-columns: repeat(4,1fr); margin: 1.8rem 0 2.7rem; border-top: 1px solid var(--cv-line); border-bottom: 1px solid var(--cv-line); }
.cv-highlight { display: grid; align-content: start; gap: .2rem; padding: 1rem .8rem; border-right: 1px solid var(--cv-line); }
.cv-highlight:first-child { padding-left: 0; }
.cv-highlight:last-child { border-right: 0; }
.cv-highlight strong { font-size: .82rem; }
.cv-highlight span { color: var(--global-text-color-light); font-size: .66rem; line-height: 1.4; }
.cv-section { margin-bottom: 2.6rem; }
.cv-section--last { margin-bottom: 1rem; }
.cv-section__heading { display: flex; align-items: baseline; gap: .65rem; margin-bottom: 1rem; border-bottom: 1px solid var(--cv-line); }
.cv-section__heading span { color: var(--global-text-color-light); font-size: .65rem; font-weight: 700; font-variant-numeric: tabular-nums; }
.cv-section__heading h2 { margin: 0 0 .5rem; border: 0; font-size: 1.15rem; }
.cv-current { display: grid; grid-template-columns: 1.25fr 1fr; gap: 1.5rem; padding: 1.1rem; border: 1px solid var(--cv-line); border-radius: 11px; background: var(--cv-soft); }
.cv-current h3, .cv-timeline h3, .cv-card h3, .cv-toolkit h3 { margin: .18rem 0 .3rem; font-size: .86rem; }
.cv-current p, .cv-timeline p, .cv-card p, .cv-toolkit p { font-size: .73rem; line-height: 1.5; }
.cv-current__meta { padding-left: 1.3rem; border-left: 1px solid var(--cv-line); }
.cv-current__meta p { margin: 0 0 .6rem; }
.cv-current__meta p:last-child { margin-bottom: 0; }
.cv-timeline { display: grid; margin-left: .3rem; padding-left: 1.25rem; border-left: 1px solid var(--cv-line); }
.cv-timeline article { position: relative; padding: 0 0 1.15rem .55rem; }
.cv-timeline article:last-child { padding-bottom: 0; }
.cv-timeline article::before { content: ""; position: absolute; top: .25rem; left: -1.61rem; width: 7px; height: 7px; border: 2px solid var(--global-bg-color); border-radius: 50%; background: var(--global-link-color); box-shadow: 0 0 0 1px var(--global-link-color); }
.cv-timeline p { margin: .12rem 0 0; color: var(--global-text-color-light); }
.cv-grid { display: grid; gap: .75rem; }
.cv-grid--two { grid-template-columns: repeat(2,1fr); }
.cv-grid--three { grid-template-columns: repeat(3,1fr); }
.cv-card { padding: 1rem; border: 1px solid var(--cv-line); border-radius: 10px; }
.cv-card p:last-child { margin: .25rem 0 0; color: var(--global-text-color-light); }
.cv-training { border-top: 1px solid var(--cv-line); }
.cv-training p { display: grid; grid-template-columns: minmax(180px,.75fr) 1.6fr; gap: 1rem; margin: 0; padding: .75rem 0; border-bottom: 1px solid var(--cv-line); font-size: .73rem; line-height: 1.45; }
.cv-training span { color: var(--global-text-color-light); }
.cv-toolkit { display: grid; grid-template-columns: repeat(2,1fr); border-top: 1px solid var(--cv-line); }
.cv-toolkit > div { padding: .75rem 1rem .75rem 0; border-bottom: 1px solid var(--cv-line); }
.cv-toolkit > div:nth-child(even) { padding-left: 1rem; border-left: 1px solid var(--cv-line); }
.cv-toolkit p { margin: 0; color: var(--global-text-color-light); }
.cv-languages { display: grid; grid-template-columns: repeat(4,1fr); gap: .65rem; }
.cv-languages p { display: grid; gap: .16rem; margin: 0; padding: .75rem; border: 1px solid var(--cv-line); border-radius: 9px; }
.cv-languages strong { font-size: .76rem; }
.cv-languages span { color: var(--global-text-color-light); font-size: .64rem; }
@media (max-width: 760px) { .cv-highlights { grid-template-columns: repeat(2,1fr); } .cv-highlight:nth-child(2) { border-right: 0; } .cv-highlight:nth-child(-n+2) { border-bottom: 1px solid var(--cv-line); } .cv-highlight:nth-child(3) { padding-left: 0; } .cv-current { grid-template-columns: 1fr; gap: 1rem; } .cv-current__meta { padding: .9rem 0 0; border-top: 1px solid var(--cv-line); border-left: 0; } .cv-grid--three { grid-template-columns: 1fr; } .cv-languages { grid-template-columns: repeat(2,1fr); } }
@media (max-width: 520px) { .cv-grid--two, .cv-toolkit { grid-template-columns: 1fr; } .cv-toolkit > div:nth-child(even) { padding-left: 0; border-left: 0; } .cv-training p { grid-template-columns: 1fr; gap: .25rem; } .cv-highlights { grid-template-columns: 1fr; } .cv-highlight { padding-left: 0; border-right: 0; border-bottom: 1px solid var(--cv-line); } .cv-highlight:last-child { border-bottom: 0; } }
@media (prefers-reduced-motion: reduce) { .cv-action { transition: none; } }
</style>
