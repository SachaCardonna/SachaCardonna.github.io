...
with $\mathbf{v} = (\eta,\mathbf{q})^t$, where $\eta$ is water total elevation, $\mathbf{q}$ is the horizontal discharge, and $\mathbf{S}\[b\](\mathbf{v}) = \mathbf{B}\[b\](\mathbf{v}) +  \mathbf{R}\[b\](\mathbf{v})$ is a topography and friction source term. 
